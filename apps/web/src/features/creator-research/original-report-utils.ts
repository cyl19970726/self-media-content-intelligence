...
export function originalReportOutline(markdown: string) {
  return originalReportBlocks(markdown).flatMap((block, index) => {
    const heading = block.match(/^#{1,6}\s+([^\n]+)\s*$/);
    return heading ? [{ href: `#original-section-${index}`, label: heading[1]! }] : [];
  });
}
