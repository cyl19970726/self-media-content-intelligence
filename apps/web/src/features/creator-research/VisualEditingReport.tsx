...
export function VisualEditingReport({ data }: { data: VideoResearch }) {
  ...
  return <section className="reader-section visual-story" id="visual-editing" aria-labelledby="visual-title">
    ...
    {visual.shotMetricBasis && <p className="metric-basis">统计口径：{visual.shotMetricBasis}</p>}

    {visual.carriers.length > 0 && <div className="visual-carriers"><header><span>画面载体</span></header>{visual.carriers.map((carrier, index) => <article key={`${carrier.name}-${index}`}><time>{timestamp(carrier.start)}–{timestamp(carrier.end)}</time><h3>{carrier.name}</h3><p>{carrier.roles.join(" · ")}</p></article>)}</div>}
    {visual.claims.length > 0 && <div className="visual-claims"><header><span>画面主张</span></header>{visual.claims.map((claim, index) => <article key={`${claim.statement}-${index}`}><time>{timestamp(claim.start)}–{timestamp(claim.end)}</time><h3>{claim.statement}</h3><p>{claim.function}</p></article>)}</div>}
    ...
    {visual.notes.length > 0 && <aside className="builder-notes"><span>Builder 补充说明</span>{visual.notes.map((item) => <p key={item}>{item}</p>)}</aside>}
  </section>;
}
