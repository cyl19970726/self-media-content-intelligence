...
function AuditState({ data }: { data: VideoResearch }) {
  return <div className="audit-state">
    <article><span>流程</span><b>{data.quality.buildState === "built" ? "Builder 完成" : data.reportFormat === "legacy_report" ? "历史材料" : data.quality.buildState}</b></article>
    ...
    <article><span>读者版本</span><b>{data.reportFormat === "legacy_report" ? "历史格式" : data.readerSummary.statusLabel}</b></article>
  </div>;
}
  ...
  return <section className="research-audit" id="audit" aria-labelledby="audit-title">
    ...
    <details className="audit-disclosure" open={defaultOpen}><summary><span>完整研究审计</span><small>来源、门禁、知识单元、逐字稿与原始报告</small></summary>
      <div className="audit-body">
        ...
        {data.quality.evaluationReadIssue && <details><summary>当前评估为何不可用</summary><pre className="legacy-report-original">{data.quality.evaluationReadIssue}</pre>{data.quality.lineage.threeLensEvaluationArtifactRef && <a href={data.quality.lineage.threeLensEvaluationArtifactRef} target="_blank" rel="noreferrer">查看所校验的评估文件</a>}</details>}
        ...
        {!data.reports.builder && data.article && <details><summary>旧版原始报告</summary><pre className="legacy-report-original">{data.article}</pre></details>}
        {(data.reports.builder || data.reports.evaluator) && <details><summary>Builder / Evaluator 原始报告</summary><div className="audit-reports">{data.reports.builder && <details><summary>Builder 原始报告</summary><OriginalReport markdown={data.reports.builder} data={data}/></details>}{data.reports.evaluator && <details><summary>Evaluator 独立评估</summary><OriginalReport markdown={data.reports.evaluator} data={data}/></details>}</div></details>}
      </div>
    </details>
  </section>;
}
