...
function AuditState({ data, workflowReviewLabel }: { data: VideoResearch; workflowReviewLabel?: string }) {
  return <div className="audit-state">
    ...
    <article><span>独立评估</span><b>{workflowReviewLabel ?? evaluationReadingLabel(data)}</b></article>
    <article><span>来源资料</span><b>{data.sourceFacts.availability.overall === "available" ? "完整" : "部分取得"}</b></article>
    <article><span>读者版本</span><b>{workflowReviewLabel ? "工作流候选报告" : data.readerSummary.statusLabel}</b></article>
  </div>;
}

export function ResearchAuditAppendix({ data, defaultOpen = false, workflowReviewLabel }: { data: VideoResearch; defaultOpen?: boolean; workflowReviewLabel?: string }) {
  ...
  return <section className="research-audit" id="audit" aria-labelledby="audit-title">
    ...
    <details className="audit-disclosure" open={defaultOpen}><summary><span>完整研究审计</span><small>来源、门禁、知识单元、逐字稿与原始报告</small></summary>
      <div className="audit-body">
        <AuditState data={data} workflowReviewLabel={workflowReviewLabel}/>
