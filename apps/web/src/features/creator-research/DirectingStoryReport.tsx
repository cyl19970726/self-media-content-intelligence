...
export function DirectingStoryReport({ data }: { data: VideoResearch }) {
  ...
  return <section className="reader-section directing-story" id="directing" aria-labelledby="directing-title">
    ...
    <dl className="directing-overview">
    ...
    {logic.informationDesign.length > 0 && <div className="builder-statement-list"><header><span>信息设计</span></header>{logic.informationDesign.map((item, index) => <article key={`${item.kind}-${index}`}><time>{timestamp(item.start)}–{timestamp(item.end)}</time><b>{item.kind}</b><p>{item.statement}</p></article>)}</div>}
    {logic.proofDesign.length > 0 && <div className="builder-statement-list"><header><span>证明设计</span></header>{logic.proofDesign.map((item, index) => <article key={`${item.proofType}-${index}`}><time>{timestamp(item.start)}–{timestamp(item.end)}</time><b>{item.proofType}</b><p>{item.statement}</p><small>边界：{item.boundary}</small></article>)}</div>}
    <div className="reader-callout"><span>信息负荷与回报</span><p><b>压缩：</b>{logic.loadAndPayoff.compression}</p><p><b>重复：</b>{logic.loadAndPayoff.repetition}</p><p><b>回报距离：</b>{logic.loadAndPayoff.payoffDistance}</p>{logic.loadAndPayoff.comprehensionCosts.map((item) => <p key={item}><b>理解成本：</b>{item}</p>)}</div>
    {logic.notes.length > 0 && <aside className="builder-notes"><span>Builder 补充说明</span>{logic.notes.map((item) => <p key={item}>{item}</p>)}</aside>}
  </section>;
}
