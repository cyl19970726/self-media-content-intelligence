...
it("shows a synthesis-only workflow with a Chinese waiting label", () => {
  const html = renderToStaticMarkup(WorkflowCreatorProgress({ value: { creatorRunId: "creator", workflowRootRunId: null, workflowRootState: null, reanalysisInProgress: false, displayedReportIsPreviousVersion: false, posts: [],
...
it("translates source checks, build, review and repair nodes", () => {
  for (const [currentNode, expected] of [["post.source-check", "来源核对"], ["creator.build", "构建中"], ["creator.review", "独立复核中"], ["creator.repair", "定向修订中"]] as const) {
    const html = renderToStaticMarkup(WorkflowCreatorProgress({ compact: true, value: { creatorRunId: "creator", workflowRootRunId: null, workflowRootState: null, reanalysisInProgress: false, displayedReportIsPreviousVersion: false, posts: [],
      counts: { total: 0, queued: 0, running: 0, built: 0, reviewed: 0, revised: 0, needsReview: 0, failed: 0, canceled: 0 }, activeSlots: 0,
