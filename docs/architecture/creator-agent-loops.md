...
一个博主完成后，不是只留下若干份视频或图文报告，而是得到：

- 全部可见帖子的表层作品地图；
- 代表性帖子的深度证据；
- 一份回答“他是谁、服务谁、为什么被关注、什么内容有效、边界是什么”的博主结论；
- 一个能回到原帖、媒体、文字、画面和版本的博主档案页面。
...
flowchart LR
    ...
    S --> K{"媒体类型"}
    K -->|视频| BV["Video Builder<br/>转写 · 画面 · 编导"]
    K -->|图文| BI["Image-post Builder<br/>正文 · 轮播 · 视觉"]
    BV --> H
    BI --> H
    H["Host 校验<br/>结构 · 引用 · 指纹"]
    ...
    class A,S corpus;
    class BV,BI,C builder;
    class H,E gate;
    class D,W result;
```

“12 条”是四个表现组各 3 个样本成员，不保证一定是 12 个不同帖子；重叠帖子只深度处理一次。
全部帖子做可观察的表层分析，代表帖子再按真实媒体类型进入视频或图文 Builder。图文 Builder 只使用
正文与已下载轮播页，不借用视频结论；缺页与不可读文字继续保留为未知。

### 四个角色

| 角色 | 负责 | 不负责 |
| --- | --- | --- |
| Host | 固定输入版本、调度、恢复、机械组装、状态与停止决定 | 发明内容含义或替 Evaluator 判断质量 |
| Builder | 按媒体类型理解视频或图文，写出知识单元、关系、证据与未知 | 传输重试、路径修补、伪造缺失证据 |
...
| 深度证据 | 代表视频或图文的 Builder 报告、Evaluator findings、版本和证据引用 |
