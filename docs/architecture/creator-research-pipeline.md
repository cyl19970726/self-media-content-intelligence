...
如果要理解“失败后为什么不能原样重试、单博主怎样闭环、多博主怎样放量”，请看
[单博主与多博主 Agent Loop](creator-agent-loops.md)。
...
flowchart TB
    subgraph FULL["阶段 A · 全量作品层：先理解这个博主发过什么"]
        ...
        A4["4. 全量作品表层标注<br/>一帖一行 · 可未归类 · 严格 ID 对齐"]
    ...
    subgraph DEEP["阶段 B · 深度证据层：再打开少量代表视频"]
        ...
        B4["10. Schema 与证据硬闸"]
        B5[("Builder 可用 / 正式已验证")]
        B1 --> B2 --> B4 -->|"Builder 合同通过"| B5
        B2 -."需要正式知识".-> B3 --> B4
        B4 -->|"不通过"| REVIEW["保留为待审<br/>写清缺什么、下一步做什么"]
    end

    subgraph SYNTHESIS["阶段 C · 博主层：按证据成熟度综合"]
        C1["11. 博主综合分析<br/>全量作品 + Builder / 已验证视频"]
        C2["12. 可选独立博主级评估"]
        C3[("13. DOSSIER_READY / WIKI_READY")]
        C4["博主档案前端投影"]
        C1 --> C2 -->|"通过"| C3 --> C4
...
- 全量层的正式产物是不可变 `portfolio-annotations`；例如 201 条可见作品必须对应 201 行，证据不足
  可以写未归类或未知，不能静默漏掉；
- **深度证据层**只选择少量代表视频，打开视频内部，分析真正讲了什么、怎么讲、怎么剪；
- 表层信息不能冒充视频内部知识；Builder 已完成但未通过独立评估的结论必须标为 `deep_provisional`，
  可以进入可审阅的博主档案，但不能进入正式跨博主知识。
...
| `creator-portfolio-annotation` | 全量作品表层信息 | 与 corpus 严格 ID 对齐的 `portfolio-annotations` | 把表层标注升级成深层事实 |
