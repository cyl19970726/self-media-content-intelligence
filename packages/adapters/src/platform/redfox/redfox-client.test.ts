...
describe("RedFoxClient", () => {
  it("keeps the credential in the server request header and counts successful requests across endpoints", async () => {
    let observedKey = "";
    ...
    expect(await client.post("/other-endpoint", { public: true })).toEqual({ ok: true });
