...
describe("production reviewer workflow composition", () => {
  it("selects new review definitions while preserving all durable historical revisions", () => {
    ...
    try {
      ...
      expect(wired.definitions.post.revision).toBe("v9");
      expect(wired.definitions.creatorSynthesis.revision).toBe("v7");
      expect(wired.definitions.creatorAnalysis?.revision).toBe("v9");
      for (const [id, revisions] of Object.entries({ "post.analyze": ["v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9"],
        "creator.synthesize": ["v1", "v2", "v3", "v4", "v5", "v6", "v7"], "creator.analyze": ["v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9"],
        "post.build": ["v1", "v2", "v3"], "creator.build": ["v1", "v2"], "post.source-check": ["v2", "v3"],
        "post.review": ["v1", "v2", "v3", "v4"], "creator.review": ["v1", "v2", "v3", "v4"],
        "post.repair": ["v1", "v2", "v3", "v4"], "creator.repair": ["v1", "v2", "v3"] })) {
        for (const revision of revisions) expect(wired.registry.resolve(id, revision), `${id}@${revision}`).toBeDefined();
      }
      expect(wired.registry.resolve("post.analyze", "v9")).toBe(wired.definitions.post);
      ...
      expect(wired.registry.resolve("post.build", "v3")?.revision).toBe("v3");
      ...
      expect(wired.registry.resolve("post.review", "v4")?.revision).toBe("v4");
    } finally { db.close(); }
  });
});
