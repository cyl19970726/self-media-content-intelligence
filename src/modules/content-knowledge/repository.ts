...
export class InMemoryContentKnowledgeRepository implements ContentKnowledgeRepository {
  ...
  getManifestByAnalysis(analysisRevisionId: string, compilerPolicyVersion: string) {
    return this.manifests.find((item) => item.analysisRevisionId === analysisRevisionId && item.compilerPolicyVersion === compilerPolicyVersion) ?? null;
  }
  saveManifest(manifest: KnowledgeContributionManifest, contributions: KnowledgeContribution[], operationKey: string, commandHash: string) {
    return this.write(operationKey, commandHash, manifest, () => { this.manifests.push(manifest); this.contributions.push(...contributions); });
  }
  ...
  saveValidation(validation: PracticeValidation, operationKey: string, commandHash: string) {
    return this.write(operationKey, commandHash, validation, () => {
      const index = this.validations.findIndex((item) => item.id === validation.id);
      if (index === -1) this.validations.push(validation); else this.validations[index] = validation;
    });
  }
