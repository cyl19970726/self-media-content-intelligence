...
import { BookOpen, Check, FlaskConical, Link2, Plus } from "lucide-react";
import {
...
export function KnowledgeDecisionPanel({ contentPackage, publication }: { contentPackage: ContentPackage; publication: PublicationRun | null }) {
  ...
  const snapshotId = `package:${contentPackage.id}:${contentPackage.updatedAt}`;

  const refresh = useCallback(async () => {
    const [nextConcepts, context] = await Promise.all([listKnowledge(), getPackageKnowledge(contentPackage.id)]);
  ...
  const currentBindings = useMemo(() => bindings.filter((item) => item.contentPackageSnapshotId === snapshotId), [bindings, snapshotId]);

  const bind = async (event: FormEvent) => {
    event.preventDefault(); if (!selectedConcept) return; setBusy(true); setError(null);
    try {
      await createKnowledgeBinding(contentPackage.id, {
        ...
        contentPackageSnapshotId: snapshotId, targetType: "concept_revision",
  ...
  const declareHypothesis = async (event: FormEvent) => {
    ...
    try {
      await createCreationHypothesis(contentPackage.id, {
        operationKey: `hypothesis:${contentPackage.id}:${Date.now()}`, contentPackageSnapshotId: snapshotId,
  ...
  return <section className="knowledge-decision-panel">
    <header><div><BookOpen size={17}/><span>KNOWLEDGE DECISION / PINNED</span></div><p>锁定这次创作依据的知识版本，并在发布前声明要验证什么。</p></header>
    ...
    <div className="decision-columns">
      <form onSubmit={(event) => void bind(event)}>
        ...
        <button disabled={busy || !selectedConcept}><Link2 size={14}/> 锁定 revision</button>
        <div className="pinned-list">{currentBindings.map((item) => <p key={item.id}><Check size={13}/><span>{item.usage} · {item.targetId}<small>{item.rationale}</small></span></p>)}</div>
      </form>
      <form onSubmit={(event) => void declareHypothesis(event)}>
        ...
        <button disabled={busy || currentBindings.length === 0}><FlaskConical size={14}/> 冻结假设</button>
      </form>
      <div className="validation-register">
        ...
        {hypotheses.map((item) => <article key={item.id}><strong>{item.statement}</strong><small>{item.baselineDeclaration}</small>
          {publication && !validations.some((validation) => validation.hypothesisId === item.id) && <button disabled={busy} onClick={() => void startValidation(item)}><Plus size={13}/> 创建复盘</button>}
        </article>)}
