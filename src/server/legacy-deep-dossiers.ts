...
const distribution = (values: number[]) => {
  ...
  return bands.map((band) => { const count = values.filter(band.test).length; return { label: band.label, count, share: values.length ? count / values.length : 0 }; });
};
...
function redWitch(): CreatorDossier | null {
  ...
  return creatorDossierSchema.parse({
    ...
    corpus: { postCount: asNumber(corpus.notes) ?? 0, videoCount: asNumber(corpus.videos), likesKnown: asNumber(corpus.videos) ?? 0,
      ...
      distribution: distributionFractions(arr(corpus.distribution).map((row) => ({ label: asString(row.label), count: asNumber(row.count) ?? 0, share: asNumber(row.share) ?? 0 }))),
...
function zhangZala(): CreatorDossier | null {
  ...
  const items = posts.map((post) => { const id = asString(post.id); const deep = deepById.get(id); const sparse = arr(deep?.sparseFrames); return {
    ...
    evidenceStatus: id === "69fe6f3a000000001a036be4" ? "deep_validated" as const : deep && fs.existsSync(path.join(researchDir, "zhang-zala-v1", "videos", id, "skill-run", "reconstruction.json")) ? "deep_built" as const : deep ? "deep_pending" as const : "surface_only" as const
  }; });
  (["high", "base", "low"] as const).forEach((tier) => items.filter((item) => item.tier === tier).forEach((item, index) => { item.tierRank = index + 1; }));
