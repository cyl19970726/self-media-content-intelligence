...
/** Select an explicitly saved source revision, never generate or patch prose in the API. */
export function readReportSource(file: string): { text: string; revision: string | null } {
  ...
  if (!fs.existsSync(manifestPath)) return { text: original.toString("utf8"), revision: null };
  const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as Record<string, unknown>;
  if (manifest.schemaVersion !== "report-source-revision@1" || manifest.originalSha256 !== digest ||
      typeof manifest.revisionId !== "string" || typeof manifest.reason !== "string") throw new Error("报告修订清单无效");
  ...
  return { text: revised.toString("utf8"), revision: `${manifest.revisionId}：${manifest.reason}` };
}
