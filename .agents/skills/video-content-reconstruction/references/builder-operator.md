...
7. 做一次读者复述检查，再回答 meta-gate：主报告若写“提出 N 项能力/步骤/配置”“给出选择框架”，必须展开原证据能支持的具体项目、含义、条件和关键例子；不能只描述作者讲过这些内容。对照知识单元与原 cue，检查核心知识是否只留在逐字稿或审计层。数字清单中的缺项、读不清的工具名或条件分别明确标未知，不用常识填补。编导镜头承诺的“观看后能理解/复述”必须能在内容还原中找到对应知识。每个 visuals 的阅读重点应具体到该画面的信息，不复制一段通用证据边界到所有画面。随后写入稳定标识 `questionId: "uncovered_information_audit"`，并回答“原视频还有哪种信息载体、意义变化或知识关系根本没被协议检查？”。展示文本可以使用任意语言，宿主不做逐字匹配。
