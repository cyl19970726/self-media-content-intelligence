...
5. `checked_unreadable` 是已执行但语义不可读的闭环状态：候选检查 evidence pack 的音轨元数据、确认当前没有模型可读的音频语义证据、明确保留 unknown，且没有借此声称音乐/音效语义，即可接受；不得额外要求模型假装聆听、分轨或识别不存在的语义证据。若候选实际遗漏了已有的模型可读载体，仍应失败。
