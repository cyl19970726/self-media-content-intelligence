...
Register every locally derived artifact used as `refType: "source"` in top-level `derivedSources` with an ID, path, kind, producing method, time range, and limitations. `path` is the exact relative filename of a real local file under the candidate root, for example `media-preparation.json` or `post-source-input.json`; do not append a JSON Pointer or `#fragment`. Keep the registered ID in `evidence.ref` and put field location/context in `evidence.supports`: for audio, use path `media-preparation.json`, ref `SRC-MEDIA-TECH`, supports `media-preparation.json#/audio: technical audio presence`. For a post title, use path `post-source-input.json`, ref `POST-TITLE`, supports `post-source-input.json#/facts/title: original post title`. A path like `media-preparation.json#/audio` is invalid even though the base file exists. If the exact local file is absent, do not register it. Schema validation checks shape only; host integrity validation also checks that every path points to an existing in-root file. Raw video/subtitle paths come from the evidence pack; unregistered labels are not evidence.
